...
### 语音控制与播报
- **语音拍照**：按住「按住说话」按钮，通过蓝牙 SCO 采集眼镜麦克风，松手后调用讯飞 SparkChain **在线**语音听写（ASR）识别；识别文本按意图分流——「二维码拍照」→ 二维码识别、「对齐拍照」→ YOLO 检测、「万用表拍照」→ 读数识别，普通「拍照」仅存原图库。
  - ⚠️ **实现说明（与旧文档不符，已更正）**：意图判定目前是 `VoiceController` 里的 **3 个关键词 + `contains` 硬匹配**；**没有**"同音字容错表"，也**没有**否定判定（"不要拍照"会被误触发，见 TODO T17）。
  - ⚠️ 本链路走**云端**讯飞 ASR；**本地离线 ASR（SenseVoice）目前只在「AI 助手」页**（`AiChatActivity`），两条链路尚未打通。
...
- 模型已由旧 `screw_detect_v2_best.onnx` 换为 **`fastener-target-p2-640.onnx`**（+ `marked-point-verifier.onnx`），检测结果在图片上绘制红框与标签（类别 + 置信度）。
