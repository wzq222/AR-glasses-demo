...
内部P2准确率基线，并已接入独立的手机相机测试页；ROI状态实验模型和3°/15°安全分诊合同仅在显式实验构建且模型就绪时接入候选点击，
但实验质量门仍为FAIL，仅供显式开启的实验阈值人工复核，不能可靠判断真实松动状态。二维码、
...
- **状态实验基线**：24张合成train-only ROI可训练并导出约1.4M参数的MobileNetV3-Small多头ONNX，
  但合成角度mean/P95为3.37°/6.27°、mask IoU为0.136，质量门FAIL。用户显式授权后，
  Android可以实验开关打包该模型；只有模型初始化就绪后点击候选才运行单ROI，边缘裁剪与训练矩形ROI一致，正式状态仍为`INSUFFICIENT`。
