...
Huawei P20 Pro (`CLT-AL00`, serial `TPC7N18604005991`). It is the new mobile performance baseline,
but it is not real-time and does not change the detector's production-accuracy status.
...
| ncnn FP32 fast, live camera after final install | 1115.4 ms total; 1033.2 ms inference | installed, still not real-time |

The fast path reduces steady full-image latency by roughly one third versus same-device ONNX, but
at about one inference per second it does not satisfy a real-time camera goal. Vulkan and trained
lightweight challengers remain required.
...
- Installed ncnn fast debug APK SHA-256:
