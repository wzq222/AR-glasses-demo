...
真实 `marked-point` 候选复核包也已加入同一证据结构，但固定为按需开关：9,567个首轮原始候选不
默认生成三套大图；只有进入有界状态复核包时，才在原有带框 JPEG 上下文之外生成无标注 PNG 原像素
裁剪、2× nearest 和 4× nearest 三个哈希绑定视图。该改动只增强人工观察，不改变候选集合、不自动
产生状态标签，也不修改既有真值。
