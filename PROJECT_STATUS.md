...
当前工作重心是以691个已初筛教师候选和151个HSV色标定位锚点为起点，逐图补全80—120个场景组的
紧固件框与色标端点真值，并在目标手机上对S/M做端到端P95基准；教师候选只用于提速，不承担整图
完整性判断。
...
基于Git外`reference-teacher-v1`复核包，人工处理39个模糊/宽框、删除1个明显误框，并逐图补齐所有
未框紧固件、核实类别；至少80个场景组完成整图复核后，并行训练PicoDet-S与M，在指定Android手机
用相同切片策略测端到端P95，选择生产模型。
