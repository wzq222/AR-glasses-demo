...
端到端P95基准；开放词汇建议框试跑已失败，人工标注将复用已确认的HSV色标作为定位锚点。
...
用Git外部YOLOv8s参考权重给100张V2代表帧生成候选，再结合已确认的151个HSV色标定位锚点人工补漏、
删错和核实类别；至少80个场景组完成整图复核后，并行训练PicoDet-S与M，在指定Android手机用相同
切片策略测端到端P95，选择生产模型。
