...
sealed test未打开、尚无跨车辆或手机证据，因此不允许表述为生产准确率。松动状态已进入真实首审与
安全融合阶段，并筛出2个疑似位移检查点，但真实数据仍没有经过隐藏二审的防松线端点和受控
`ALIGNED/DISPLACED`成对真值，因此尚不能训练或宣称可靠状态模型。
...
先对真实首审中的`ref-09/ref-13`做隐藏结论端点二审；随后采集同一检查点受控`ALIGNED/DISPLACED`
成对照片，标注固定侧/活动侧两段防松线端点、可见性和维护人员确认状态，并用真实成对数据标定阈值。
