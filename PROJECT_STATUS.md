...
- 2026-09-04：生成带内置案例图库的新Debug APK。构建时从Git外
  `app-original-gallery`读取二维码1张、防松线1张、万用表5张，仅写入Gradle生成资产和APK，
  现场图片未进入Git。App原图库现会在后台幂等释放内置案例，并与眼镜同步/用户导入图片合并展示，
  SOP可直接选择。78项Android测试和干净构建通过，APK为111,263,539 bytes，SHA-256为
  `EC9FF9F054605EF94D474C28672EF2EFE3864F5584027222172BB33A676AA8D0`；APK内7张图逐文件哈希均与
  源文件一致。最终ADB检查无在线设备，故本轮未完成覆盖安装和真机UI冒烟。模型、阈值和formal truth
  均未修改。证据见`docs/validation/2026-09-04-bundled-case-gallery-apk.md`。

- 2026-09-04纠偏：此前手机冻结集`75/75`采用宽松“候选中心落入真值/低IoU/包含率”覆盖口径，
  不能代表检查点被准确框出。改用一对一匹配后，当前手机最终框在IoU 0.30仅62/75、IoU 0.50
  仅40/75，严格定位门为FAIL；当前APK只作流程与推理联调，不能作为准确定位交付。下述旧
  `75/75`记录均应按宽松候选覆盖理解。纠偏代码在`feature/full-image-fastener-vision`提交
  `8e8631b`。Android显示层已加入保守巨框抑制：大框面积至少为内部紧框4倍且覆盖内部框80%时，
  只显示紧框；冻结17图候选293降至254，严格IoU 0.30/0.50覆盖保持62/75和40/75不再下降。
  74项Android测试和Debug构建通过，APK SHA-256为
  `E2292CCD277442B12938457E0E5EA6CE46693269F28B95089B0C0F24525559D4`，已安装并启动于
  华为`NOH-AN01`。这只减少显然的重叠巨框，后续仍须用紧框和巨框/重复框硬负样本重训模型。

- 2026-09-04：定位到手机稀疏检出的直接回归：默认ncnn阈值被设为`0.20`，而冻结高召回链
  的校准值为`0.0019424824276939034`。已恢复校准值并加入合同测试；新APK在华为`NOH-AN01`
  上覆盖安装后，真机冻结17图运行得到294个候选；按当时宽松口径为17/17场景、75/75真值覆盖，
  P50 0.732秒/图。严格定位纠偏后该模型为FAIL；该运行只证明链路可执行，不证明准确定位。
  另对全图+切片重叠框建立Git外源感知融合原型，4项测试通过，困难图52框仅合并6个有几何证据的重复框至46框，
  不以提高置信度阈值牺牲召回。证据见`docs/validation/2026-09-04-high-recall-threshold-recovery.md`。
- 2026-09-04：SOP 每步现提供“拍摄本步骤证据 / 从手机图库选择 / 从 App 原图库选择”三个图片入口，
  均复制为独立证据快照后进入原有二维码、防松线、万用表分析、人工确认和上传链；支持
  JPEG/PNG/WebP、25 MiB 输入门、2200 像素有界解码和 EXIF 旋转，取消或失败不覆盖已有证据。
  三包 Git 外案例已导入真机 App 原图库（二维码1、防松线1、万用表5），案例文件未进入仓库。
  真机三步 run 使用手机图库1张、App 原图库2张完成3个步骤和3份证据并提交后台；后台核验
  `evidence_source`与媒体类型正确。发现并修复提交后仍显示“继续巡检”且可重复开 run 的状态缺陷，
  生产服务现返回三步任务`submitted`、待办1项、待审核1项并拒绝重复启动（HTTP 409）；手机复验只显示
  十步任务1项。Android 70项、Server 12项测试通过。困难案例同时暴露准确性边界：防松线图检出0点，
  233.5V万用表图未读出，均由人工选择“无法判断”；不能据此宣称算法验收通过。证据见
  `docs/validation/2026-09-04-sop-gallery-evidence-and-live-run.md`。
...
- 2026-09-04：API 36本地模拟器通过ARM64转译成功安装APK，首次冷启动暴露无音频设备上
  ...
  再更换为可用数据线后，`NOH-AN01`已正常进入ADB；最新APK完成覆盖安装，冷启动2.342秒，进程持续
  存活，主界面入口及“中车巡检工作台”登录页完成UIAutomator核验，AndroidRuntime致命日志为0。
- 2026-09-04：使用临时提供且未落盘的管理员凭据完成生产鉴权复验。手机端经HTTPS成功登录并同步
  `CRRC-DEMO-010`十步和`CRRC-DEMO-001`三步共2条待执行任务；独立浏览器登录公网管理后台后显示
  1个有效用户、3个SOP版本、2个待执行任务，并核验十步流程确含10步和5个防松线人工审核点。
  唯一控制台错误为不影响业务的`/favicon.ico` 404，验证后浏览器已退出登录并关闭。
