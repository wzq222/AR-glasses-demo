...
校准`safe-auto-v2`的2至4张代表图，再按100个独立代表场景执行高清漏检扫描、候选审核和补框盲审。
...
先对Git外`review-packs/safe-auto-v2`做2至4张校准审核，验证补框规范化坐标、逐项盲审和最终COCO
组装；通过后优先完成至少64个train和16个val独立场景，再并行训练PicoDet-S与M。
