...
- 2026-09-04：纠正了防松检查点定位的评估口径。旧 `marked-point-model-gate-v2` 允许
  “候选中心落入真值 / IoU >= 0.10 / 包含率 >= 0.50”任一成立，且一个候选可覆盖多个真值，
  因此此前 `75/75` 只能表示宽松覆盖，不能表示准确定位。现已升级为确定性一对一
  `IoU >= 0.30` 的 `marked-point-model-gate-v3`，复核训练也不再把一个巨框复用为多个检查点
  正样本。严格重算后，当前512原始候选为68/75（IoU 0.30）和62/75（IoU 0.50），当前手机
  复核后为62/75和40/75，故当前移动链路定位门明确FAIL；e2与v9教师并集在IoU 0.30达到
  75/75，可用于生成紧框与硬负样本后蒸馏新的移动模型，但不能直接视为交付模型。证据见
  `docs/validation/2026-09-04-marked-point-localization-gate-correction.md`。
...
- 2026-09-01：用户显式授权使用失败质量门的Git外ROI ONNX作实验阈值提示。Android现仅在
  `CRRC_WITNESS_STATE_MODEL_DIR`与`CRRC_WITNESS_STATE_EXPERIMENTAL=1`同时存在时打包指定SHA-256的
  `witness-roi.onnx`；候选点击后冻结检测帧、执行单ROI、显示点角度/±6.3°警示/3°与15°
  分档/耗时，对话框关闭前暂停候选推理。静态元数据兼容固定ONNX的符号输出维，运行数组仍严格验形；边缘ROI按训练规则独立夹取为可能的矩形。`quality_logits`因训练目标恒定只作接口校验，拒判改用热图峰值集中度、mask局部支撑和joint几何一致性。默认构建和模型初始化失败均不安装点击监听并显示对应拒判/不可用文案。
  该路径仍不是生产准确率，不输出正式`ALIGNED/DISPLACED`；用户取消了采集和人工记录落盘。
...
2026-09-04严格定位口径复算后，当前512单类ncnn+128复核在17张同源开发图上只有
62/75（IoU 0.30）和40/75（IoU 0.50），并存在重复框和大区域框，因此当前模型严格门为FAIL，
不得沿用旧口径的75/75结论。e2与v9教师并集在IoU 0.30达到75/75，可用于重新产生紧框训练样本，
但不能直接交付到手机。当前手机链只证明推理能够运行，XNNPACK端到端P50为1.370秒，仍未达到
流畅视频实时。松动状态已完成真实首审和
2个疑似点的隐藏二审；两个疑似点均因宽涂层或低分辨率落为`INSUFFICIENT`。当前ROI状态实验模型
已经以显式开关接入Android候选点击，但严格质量门仍失败，只允许展示待人工确认的实验提示。真实数据仍没有受控
`ALIGNED/DISPLACED`成对真值，因此不能宣称可靠状态模型。现有审计链已能阻止单张历史图被错误
提升为松动真值。

2026-09-04 用户要求暂时停止模型修改。E5/E6 研究实验均未提升为交付权重：E5 四切片训练仍只
覆盖70/75；E6在极低阈值下可覆盖75/75，但代价为161.5候选/图，后接现有verifier降至
56.4候选/图时又漏到72/75。结论是重框问题不能靠继续调NMS解决，下一次恢复实施应先补独立场景的
紧框/中心、标记掩膜及宽框硬负样本，再把纯分类复核器替换为定位感知的ROI模型。详细证据见
`docs/research/2026-09-04-marked-point-overlap-root-cause.md`。
...
模型与APK修改当前暂停。恢复前先补至少34个独立train场景，并补齐marked-point紧框/中心、
防松标记掩膜以及巨框、重复框、邻接螺栓和lookalike硬负样本。恢复后采用高召回区域提议 +
定位感知ROI头，而不是继续提高NMS强度或沿用纯分类verifier；内部门采用一对一IoU 0.30召回与
完整场景率，同时报告IoU 0.50定位质量和每图候选负担。通过内部门后，
再采集100–150个跨车辆、跨手机、跨光照的完整独立场景，人工补全后一次性打开sealed test；
性能分支只有在保持该独立门时才评估INT8或更小检测器。状态分支停止继续在24张合成ROI上调参，
先采集至少10个真实物理检查点的受控角度组，覆盖
`0/2/3/5/8/10/15/20°`、正视/左右斜视、两种距离和明暗光照；优先完成至少200个高质量ROI的
固定侧、活动侧、防松线、接缝与四关键点标注，并按物理点隔离分区。当前Android接入仅为实验阈值人工复核；
手机单ROI冷/热P50、P95、内存和10分钟稳定性尚未验证。独立物理点角度门通过后才能评估生产状态能力。
...
- `docs/validation/2026-09-04-marked-point-localization-gate-correction.md`：旧宽松覆盖口径纠偏、
  一对一IoU门、当前手机链严格失败和教师并集重训依据。
